...
class Data:
    ...
    def _convert_sentence_pair_to_bert_dataset(
        ...
        for i, _ in tqdm(enumerate(s1_list), ncols=80):
            ...
            self.truncate_seq(sc_tok, 290)

            sc_leave_space = 290 - len(sc_tok) if len(sc_tok) < 290 else 0

            self.truncate_seq(bc_tok, 219+sc_leave_space)


            sc_len = len( sc_tok )
            bc_len = len( bc_tok )

            tokens = ['[CLS]'] + sc_tok + ['[SEP]']

            segment_ids = [0] * len(tokens)

            tokens += bc_tok + ['[SEP]']

            segment_ids += [1] * ( len( bc_tok ) + 1 )
            ...
            all_sc_len.append(sc_len)
            all_bc_len.append(bc_len)

        all_input_ids = torch.tensor(all_input_ids, dtype=torch.long)
        all_input_mask = torch.tensor(all_input_mask, dtype=torch.long)
        all_segment_ids = torch.tensor(all_segment_ids, dtype=torch.long)
        all_sc_len = torch.tensor(all_sc_len, dtype=torch.long)
        all_bc_len = torch.tensor(all_bc_len, dtype=torch.long)


        if label_list:  # train
            all_label_ids = torch.tensor(label_list, dtype=torch.long)
            return TensorDataset(
                all_input_ids, all_segment_ids, all_input_mask, all_label_ids, all_sc_len, all_bc_len)
        # test
        return TensorDataset(
            all_input_ids, all_segment_ids, all_input_mask, all_sc_len, all_bc_len)
