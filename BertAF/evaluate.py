...
def evaluate(model, data_loader, device) -> List[str]:
    ...
    for batch in tqdm(data_loader, desc='Evaluation', ncols=80):
        ...
        input_ids, segment_ids, input_mask, sc_len, bc_len = batch

        with torch.no_grad():
            logits = model(input_ids, segment_ids, input_mask, sc_len, bc_len)
...
if __name__ == '__main__':
    ...
    acc, f1_score = eval_file(
        '../work/lbwj_bin/data/test.csv', 'output/result1.csv')
